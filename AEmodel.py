...
from tensorflow.keras.layers import   Dense,Conv2D,MaxPool2D,Activation,Flatten,AveragePooling2D,LayerNormalization
import os 

class AE_model(Model): 
    # 评估网络,输出动作
    def __init__(self):
        super().__init__()
        # resnet
        ...
        self.c_2_1 = Conv2D(filters=64, kernel_size=(3, 3),strides=1, padding='valid',activation='elu',
                            kernel_initializer='he_uniform')
        ...
        self.c_3_1 = Conv2D(filters=128, kernel_size=(3, 3),strides=1, padding='valid',activation='elu',
                            kernel_initializer='he_uniform')
        ...
        self.c_4_1 = Conv2D(filters=256, kernel_size=(3, 3),strides=1, padding='valid',activation='elu',
                            kernel_initializer='he_uniform')
        ...
        self.f1_2 = Dense(32, activation='elu', kernel_initializer='he_uniform')
        # 加载网络
        self.checkpoint_save_path = "./model_Encoder/base"
...
class Class_model(Model): 
    ...
    def __init__(self):
        super().__init__()
        # resnet
        # self.f1 = Dense(128, activation='elu', kernel_initializer='he_uniform')
        ...
        # 加载网络
        self.checkpoint_save_path = "./model_Encoder/class"
        if os.path.exists(self.checkpoint_save_path + '.index'):
            print('-------------load the model-----------------')
            self.load_weights(self.checkpoint_save_path)
        else:
